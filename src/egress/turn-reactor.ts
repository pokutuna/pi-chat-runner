...
//
// docs/design/ingress-egress.md §7: リアクションは Reply とは別の経路で返る。発生源は
// Runner の観測 (agent_end の中身) で、Runner は start/ok/error という platform 非依存の
// 状態を発するだけ。chat 表現 (Slack ならリアクション絵文字) への写像は platform 実装
// (src/egress/slack/) が持つ — ingress の Ingress vs IngressAdapter と同じ直交。

/** Turn の視覚状態。start=処理開始 / ok=完走 / error=失敗。
 * 判定結果 (pi-events.ts TurnStatus = "ok" | "error") はこの部分集合。 */
export type ReactionState = "start" | "ok" | "error";

/** メッセージ単位で「その Turn の状態」をユーザーへ視覚的に返す port。
