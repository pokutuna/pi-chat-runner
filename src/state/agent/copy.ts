// ファイルコピーによる Agent State の archive の実装 — docs/design/state.md §5, §7
//
...
/** sessionKey (`<channelId>:<threadTs>`) を archive のパスに変換する。
 * `:` を `/` に置き換えると state.md §6 の `<workdirBase>/<channelId>/<threadTs>/`
 * と揃う。 */
function archivePath(baseDir: string, sessionKey: string): string {
...
export class CopyWorkdirStore implements WorkdirStore {
  ...
  async restore(sessionKey: string, workdir: string): Promise<boolean> {
    const archive = archivePath(this.baseDir, sessionKey);
    const entries = await readEntriesOrEmpty(archive);
    ...
    for (const entry of entries) {
      await copyRegularEntry(archive, workdir, entry, stats);
    }
  ...
  async flush(sessionKey: string, workdir: string): Promise<void> {
    ...
    for (const entry of rest) {
      await copyRegularEntry(workdir, archive, entry, stats);
    }
    if (entries.includes(SESSION_FILE)) {
      await copyRegularEntry(workdir, archive, SESSION_FILE, stats);
    }
    logCopy(this.logger, "workdir flush", { sessionKey }, started, stats);
  }
}

/** archive のサイズがこれを超えたら warn する既定値 (state.md §5.1: ガードレールでは
 * なく気づきのため。想定は memory/skills/小さなドキュメントで数 MB オーダー、
 * その 10 倍程度を「気づくべき」ラインとする)。 */
const DEFAULT_SHARED_SIZE_WARN_BYTES = 50 * 1024 * 1024;

/** ファイルコピーのみによる SharedStore 実装。archive は `<baseDir>/<channelId>/`。 */
export class CopySharedStore implements SharedStore {
  ...
  async restore(channelId: string, dest: string): Promise<void> {
    const archive = join(this.baseDir, channelId);
    const entries = await readEntriesOrEmpty(archive);
    ...
    for (const entry of entries) {
      await copyRegularEntry(archive, dest, entry, stats);
    }
  ...
  async flush(channelId: string, src: string): Promise<void> {
    ...
    for (const entry of await readEntriesOrEmpty(src)) {
      await copyRegularEntry(src, archive, entry, stats);
    }
   ...
   * 運用者が気づけるようログだけ出す。
   *
   * 判定は「今 flush した staging の総バイト数」= コピー中に数えた値で行い、archive を
   * 走査し直さない。archive は staging のコピーなので概算として十分で、走査は archive
   * (FUSE の場合ネットワーク越し) のファイル数に比例するコストを毎ターン払うことになる —
   * 気づきのための警告に対して高すぎる。削除が伝播しない分だけ archive は staging より
 ...
 * 支配される — archive が GCS FUSE の場合 1 ファイルが 1 オブジェクト書き込みになる
 * ため。どちらが効いているか切り分けられるよう両方記録する。 */
interface CopyStats {
  files: number;
  bytes: number;
}
