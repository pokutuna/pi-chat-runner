...
REPL アダプタ (src/ingress/local/repl.ts — dev:local のみが使う)
  - stdin 1 行 → 文法パース (純関数) → core の API 呼び出し
  - core の変化通知 → stdout 描画
  - 現在チャンネル・現在ユーザー・DM フラグなどの「今の状態」はアダプタ側が持つ
    (core はステートレスにイベントを受けるだけ)
```
...
すべてのメッセージ (入力・bot 投稿とも) に `[N ts]` を付けて表示する。
bot 投稿はスレッド位置がわかる形で出す:

```
[1 1752800000.000001] you: @bot このアラート調査して
...
- `updateMessage` (進捗通知の上書き) は `(update [N])` として再表示する
- `postMessage` の `files` はパスの列挙のみ (アップロードはしない)
...
Node >= 26 の built-in で全て賄い、runtime 依存は追加しない:

- REPL 入力: `node:readline` (履歴・行編集・completer)
- 色付け: `node:util` の `styleText` (NO_COLOR / TTY 判定込み。chalk 等は不要)
