...
| `!channels` | 設定ファイルの channels ブロックのエントリ一覧を表示 (現在の投稿先に `(current)` を付与) |
...
| `!help` | メタコマンド一覧など詳細ヘルプを表示 (起動時は簡潔な案内のみを表示し、詳細はこのコマンドへ誘導する) |
...
- 最下行が入力欄。Enter で確定した行は repl-logic.ts の `handleLine` に渡す。
  ...
  補完する (それ以外の Tab はフォーカス切替)。有効なメタコマンド (`!help`
  等) は入力中に `!COMMAND` 部分が色付く
- フォーカス: Tab / Shift-Tab で input → log → chat を巡回し、Escape で
  input に戻る。フォーカス中のペインは枠線の cyan とタイトル末尾の ` *`
  マーカー (色が出ない環境向け) で示す。入力欄はフォーカス中だけプロンプト
  が cyan になり、カーソルの反転セルを表示する (非フォーカス時は反転を消し、
  どこにフォーカスがあるか一目でわかるようにする)。input フォーカスでは
  文字入力・カーソル移動 (←→ / C-a C-e 等の readline 風バインド)・入力履歴の
  呼び出し (C-p / C-n = previous/next-history。履歴を辿り始めるときの下書きは
  退避され、末尾を越えて C-n すると復元される) が効き、log/chat フォーカスでは
  スクロールのみ効く
