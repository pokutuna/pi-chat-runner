...
### ユーザー CLI のための allowlist 拡張 (PI_ENV_PASSTHROUGH)

利用者が拡張イメージに CLI を追加すると token が必要になる場合がある
(例: gh に GH_TOKEN)。宣言的な穴あけとして: デプロイ時に `--set-secrets` で
env に載せ、`PI_ENV_PASSTHROUGH=GH_TOKEN,FOO_API_KEY` (カンマ区切り) に
列挙された名前だけを allowlist に加えて pi へ継承する。

安全弁: `SLACK_` / `BRIDGE_` など bridge 予約 prefix の名前は列挙されても
コードで拒否する (誤設定で bot token が pi に漏れる事故を防ぐ)。

secret の値そのものは Secret Manager にのみ存在し、YAML (service.yaml) に
書かれるのは参照だけ。ChannelDoc には一切置かない ([config.md](config.md) §5)。
...
| パス | 実体 | 役割 |
|---|---|---|
| `/tmp/sessions/<threadTs>/` | tmpfs (インスタンスローカル) | workdir。pi の cwd。使い捨て |
...
## §4 追加メッセージの受け渡し — ポーリングして RPC で配達

この仕組みで「ターン境界ポーリング」は 3 つの仕事に分解される:
**配達 = ホストのタイマーループ、注入 = pi 内部の 2 段キュー、inbox 削除 =
agent_end 後の flush**。ホストがターン境界を検出する必要はない — 「境界で
注入する」責務は pi が持っているので、ホストは見つけ次第 stdin に流すだけで
よい。

ホストは pi 実行中、**一定間隔 (env `INBOX_POLL_INTERVAL_MS`, 既定 5000) で
inbox を get()** し、新着があれば pi の stdin に RPC コマンドとして書く:

```typescript
// inbox ポーリングループ (pi 実行中も linger 中も同じ)
for (const item of await inbox.fetchNew()) {
  pi.stdin.write(JSON.stringify({
    type: item.mode,             // "steer" | "follow_up"
    message: renderEvent(item.event),
  }) + "\n");
}
```

**注入タイミングは pi が管理する**のでホストは配達するだけでよい: steer は
「次の LLM 呼び出しの前」(ステップ境界 = ツール実行の合間)、follow_up は
「現ターン完了後」に pi 内部の 2 段キューが処理する。

realtime リスナ (onSnapshot) を使わない判断 ([session-model.md](session-model.md) §4) は不変。ポーリングは
「Firestore を耐久キューとしてだけ使う」ための選択で、配達遅延は最大で
ポーリング間隔 (既定 5s)。steer が効くのは次のステップ境界なので、この遅延は
実用上見えない。

処理済み inbox doc の削除はターンの flush 後 (§3)。ポーリングで拾った時点では
削除せず、doc ID をメモリ上で既読管理する (再取得しても冪等)。

idle 判定: agent_end 受信 + inbox 空 + バックグラウンド作業なし → linger へ。

ポーリング間隔は env で調整可能 (`INBOX_POLL_INTERVAL_MS`, 既定 5s)。steer が
効くのは pi の次のステップ境界 (ツール実行の合間) なので、間隔を 5-10s に
伸ばしても体感差はほぼ無い。短くする動機は「追加指示への反応の速さ」、
長くする動機は Firestore read の節約で、チャンネル単位でなくデプロイ単位の
調整で足りる ([config.md](config.md) §5 の env 行と同じ扱い)。
