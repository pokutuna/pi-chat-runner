...
- ターンにタイムアウトを設け、超過したら pi を kill (プロセス使い捨て設計なので kill してよい)。異常終了はコマンド失敗・クラッシュと同じく、このターンで prompt 済みだった入力は ack して捨てる (retry しない。session-model.md §6) — 同じ入力は再び timeout する可能性が高く、残すと毒ループになるため
