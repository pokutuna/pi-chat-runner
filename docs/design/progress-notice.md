...
| thread_key ごとの直列化 | `EgressRouter` は `notifyProgress`/`deliver`/`clearProgress` を進捗キー単位の FIFO キューで直列化する。進捗タイマーの tick と `reply` の配送は別々の非同期経路から発火するため、キューを介さず `progressMessageIds` を直接読み書きすると、配送済みの `reply` の後に古いスナップショットの tick が新規メッセージを再投稿してしまう |
