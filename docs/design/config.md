...
channels:
  ...
  - channel: "#alerts"      # apply 時に Slack API で channelId に解決 (ID 直書きも可)
...
`channels.yaml` とは別ファイルに保つ。性質が違うため:

| | agent.yaml | channels.yaml |
|---|---|---|
| 対象 | bridge プロセスの挙動既定 (全チャンネル共通) | チャンネルごとの振る舞い (default + 差分) |
...
agent.yaml には provider / turnTimeoutMs / envPassthrough のような**モデル以外の
bridge 挙動既定**だけが残る。

優先順位は **env > agent.yaml > コード既定**。env は「ローカルで一時的に変えて
試す」ための override として残す (PI_PROVIDER / TURN_TIMEOUT_MS / PI_ENV_PASSTHROUGH
が対応)。モデルは agent.yaml にも env override にも持たない — pi 本体のモデルは
channels.yaml の default.model / channel.model、classifier のモデルは trigger.when の
各ノードの model に一本化する (§2.3)。boot 時に strict に validate し、未知キーは
...
### 実効設定の書き出し (effective config)

`default` と channel をマージした後、あるチャンネルで**実際に効く設定 (実効設定)** を
確認できる手段を持つ。マージ規則 (§2.2) を頭の中で追わずに、結果を目で見られること。

- **解決関数はランタイムと共有する**。dump 専用の別実装を持たず、イベント処理で使う
  `resolveChannelConfig(channelId)` (agent.yaml + channels.yaml のロード → default 特定 →
  merge) をそのまま呼ぶ。dump と本番で結果がずれないことを保証する。
- **出所 (provenance) を併記する**。各フィールドが `default` 由来か channel 由来か、
  agent.yaml 由来かコード既定 (fallback) かを付記する。「なぜこの model になったのか」が
  マージ規則を再現しなくても分かる。
- 人向けの整形出力と、機械可読な `--json` の両方を出す。CLI サブコマンド
  (`... config show <channel>` 相当) から呼ぶ。

```
$ agent-config show '#alerts'
channel: #alerts  (resolved id: C0XXXX)
model:            gemini-3-pro            ← channels.yaml #alerts
systemPrompt:     (from ./prompts/alerts.md, 1.2KB)   ← channels.yaml #alerts
trigger.when:     OR[ keyword, AND[classifier(gemini-3.1-flash-lite), classifier(code default)] ]
                                          ← channels.yaml #alerts (replaced)
session.mode:     thread                  ← default
```
...
| bridge の挙動既定 (provider / turn timeout / envPassthrough。モデルは含まない) | 設定リポジトリ | CONFIG_DIR/agent.yaml (§6) | YAML |
...
Gate はこの基盤が提供する type 名 (`kind`) から選び、設定値を添える。apply 時 (§6) に
