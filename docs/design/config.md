...
| lease TTL / linger / debounce / inbox ポーリング間隔 | env | チャンネルごとに変える動機がまだ無い。生じたら §2 に昇格 |
...
```
agent-config/
  agent.yaml            # デプロイ/app 設定: project, region, bucket, 既定 model, 既定 image
  channels/
...
| app 既定 (poll 間隔・lease TTL・既定 model) | デプロイ | service.yaml の env | YAML |
...
| ユーザー CLI 用の secret (例: GH_TOKEN) | デプロイ | --set-secrets + PI_ENV_PASSTHROUGH ([session-runtime.md](session-runtime.md) §2) | YAML (service.yaml) |

bridge 自体の設定に独自の設定ファイルは発明しない。Cloud Run の service.yaml が
すでに YAML であり、secret は secretKeyRef による参照で書けるため「平文 secret を
repo に置かない」制約とも両立する。これで「実行環境以外のカスタマイズは YAML で
表現できる」が全層で成立する。
...
| ChannelDoc への secret 記載 | Firestore に平文 secret を置かない方針 (§5) | Secret Manager + --set-secrets + PI_ENV_PASSTHROUGH ([session-runtime.md](session-runtime.md) §2) |
