...
| 置き場所 | 何を置くか | 変更の手段 |
|---|---|---|
| **env** | `agent.yaml` が `${env.X}` で参照する値の source (Slack token・store backend 等) + boot 前提 (CONFIG_DIR / LOG_LEVEL)。secret は値でなく env 名で拾う | デプロイ (Cloud Run env + `--set-secrets`) |
| **コンテナイメージ** | 実行能力: pi 本体、CLI ツール (gcloud 等)、**skill**、reply/permission-gate extension、焼き込みリポジトリ | イメージビルド + デプロイ |
| **agent.yaml (設定リポジトリ)** | bridge 本体の挙動宣言: connector (接続) / store (永続化) / pi・agent (実行環境)。boot 時に 1 回直読み。値は `${env.X}` 参照で env と結ぶ | ファイル編集 + 再起動 |
| **channels.yaml (設定リポジトリ)** | 振る舞いのテキスト: プロンプト、起動条件、モデル (初期版はこれだけ。能力の選択は将来拡張 §2) | ファイル編集 (デプロイ不要) |
...
チャンネル設定は**振る舞いのテキストと trigger だけ**を持つ。能力 (skill / CLI /
extension) はイメージに焼き、チャンネルごとの選択はしない — 1 サービス 1 イメージなので
選択の余地自体を作らない。

チャンネル設定は 1 ファイル `channels.yaml` に**配列**で並べ、先頭に**必ず `default`**
エントリを置く。実行時は常に **`default` + そのチャンネル固有エントリ** をマージした
1 つの ChannelDoc で動く (§2.2 マージ)。「どの値がどこから来たか」がファイル上で
1 対 1 に読めるよう、フィールド構造は default とチャンネルで完全に同じにする。

```yaml
# channels.yaml — 配列。先頭の default は必須、以降は channel ごとの差分
channels:
...
**それでも実効設定は dump で確認できる** (§6 実効設定の書き出し)。channels.yaml は
...
- pi の内部仕様に channels.yaml の中身が結合し、pi 更新のたびに全チャンネルの doc が壊れうる
...
| 実行時 (セッションごと) | ChannelDoc の差分 (プロンプト・model) + transcript | channels.yaml 直読み (FileConfigSource) → pi の spawn 引数 |
...
const pi = spawn("node", [
  ...
  // --skill は配線しない — skill は pi 既定パス ($AGENT_HOME/.pi/agent/skills) に
  // 置けば pi が自動ロードする (§3)
], { cwd: workdir });
...
| Slack token / EventSource 切替 | `agent.yaml` の connector.slack (値は `${env.X}` 経由) | チャンネルでなく接続の属性。§6 で connector に集約 |
...
| 環境変数 / イメージに焼く | 不変で安全 | 「デプロイなしで変えられる」利点が消える | channels.yaml の存在意義と矛盾 |
...
agent-config/
  ...
  channels.yaml         # 全チャンネル設定を配列で 1 ファイルに (先頭 default 必須。§2)
  prompts/
    alerts.md           # 長文プロンプト (YAML から参照)
```

チャンネルを 1 ファイルにまとめるのは、**default と各チャンネルの対応関係を 1 画面で
見比べられる**ようにするため。ファイル分割は求めていないので (§2 のマージが「default +
channel」の 1 段だけ)、束ねる方が読みやすい。
...
bridge 本体の設定は CONFIG_DIR 直下の `agent.yaml` に書く。connector (接続) → store
(永続化) → pi・agent (エージェント実行) を「**入口 → 出口**」の順に 1 ファイルへ並べる。
`channels.yaml` (チャンネルごとの振る舞い) とは別ファイルに保つ。性質が違うため:
...
| 解決 | 3 モジュール (connector / store / agent) が同じファイルを読み、各自のトップレベルキーだけ取り出す | FileConfigSource がイベントごとに読む |

値には `${env.X}` / `${env.X:-default}` で環境変数を参照できる (下記「`${env.X}` 参照」)。
secret は値を直書きせず `${env.X}` で env (Secret Manager 由来) を拾う。同じ書き方で
「値直書き」も「env 注入」も表現でき、YAML 側で `${env.X:-default}` に一本化すると読みやすい。

```yaml
# agent.yaml — connector (接続) → store (永続化) → pi/agent (実行) の順。
# 値には ${env.X} / ${env.X:-default} を書ける。secret は ${env.X} で env を拾う。
...
store:
  ...
  sqlitePath: ${env.SQLITE_PATH:-/tmp/pi-chat-runner/state.db}  # backend: sqlite 用
...
| skill の登録 | イメージ (pi 既定パス $AGENT_HOME/.pi/agent/skills) | pi が自動ロードする (§3)。Runner の `--skill` 配線は無い |

### `${env.X}` 参照

agent.yaml の値には `${env.X}` を書ける (`src/config/env-ref.ts`)。secret を平文で
置かず env (Secret Manager 由来) から拾う・デプロイ先ごとに変わる値を YAML から切り出す、
の両方をこの 1 記法でまかなう:
...
- **対象は channels.yaml だけ**。dump は `resolveChannelConfig` (channels.yaml のロード →
  default/dm 特定 → merge) を呼び、`agent.yaml` (connector / store / pi / agent) は**読まない**。
  したがって agent.yaml の `${env.X}` secret が dump 出力に解決済みで現れる経路は無い
  (dump は secret を触らない — 漏洩面が構造上存在しない)。
...
| bridge の挙動既定 (provider / turn timeout / agent.runtime。モデルは含まない) | 設定リポジトリ | agent.yaml の pi / agent.runtime (§6) | YAML |
| Gate の選択 + 合成 + 設定値 (分類 criteria・classifier モデル) | チャンネル | channels.yaml の trigger.when | YAML |
| 初期プロンプト・文脈 | チャンネル | channels.yaml + prompts/*.md 参照 | YAML + Markdown |
| pi (agent) の利用モデル | チャンネル | channels.yaml の model (default / channel) | YAML |
| CLI・コマンド追加 / pi-extension | 実行環境 | Dockerfile (FROM base + `pnpm install --prod`, [session-runtime.md](session-runtime.md) §5) | コード (イメージ) |
| skill | 実行環境 | イメージの pi 既定パス ($AGENT_HOME/.pi/agent/skills)。pi が自動ロード (§3) | コード (イメージ) |
...
bridge の設定の置き場は CONFIG_DIR/agent.yaml (§6)。「service.yaml を bridge の
...
明文化される。YAML をどう書いても新しいコード実行経路は生えない (criteria/pattern は
...
| channels.yaml への secret 記載 | YAML に平文 secret を置かない方針 (§5) | Secret Manager + `--set-secrets` (値) + agent.yaml の `agent.env` に env 名を明示列挙 ([session-runtime.md](session-runtime.md) §2) |
