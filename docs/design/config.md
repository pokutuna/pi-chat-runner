...
| DM — お試し・個人利用 | passthrough (`dm` エントリで明示オプトイン。既定は disabled) | 既定 | 既定 | 既定 |
...
- チャンネル間で変わるのは **trigger / image / skill・MCP の有効化 / テキスト / model の 5 種だけ**。
  ...
  → それらを Config に入れない根拠
- **ChannelDoc が無くても動く** (#ask-ai は doc なしの既定動作)。DM は doc なしでは
...
channels:
  - channel: default        # 予約名。全チャンネルの土台。ここだけは省略不可
    model: google-vertex/gemini-3.5-flash # pi (agent) のモデル既定 (§2.3 の canonical 形式)
    trigger:
      when:
        - kind: mention     # 既定は mention のみ (§7 trigger.when)

  - channel: "#alerts"      # Slack API で channelId に解決 (ID 直書きも可)
    model: google-vertex/gemini-3-pro     # default.model を上書き
...
interface ChannelDoc {
  ...
  model?: string;          // pi (agent) のモデル。canonical "provider/model-id[:thinking]" 形式 (§2.3)
...
`memory` は skill の一種だがパス参照ではなく boolean — 実体は基盤同梱の組み込み
skill (`builtin-skills/memory/`) で、shared (env `SHARED_DIR`,
[shared.md](shared.md)) 有効時に既定で `--skill` に配線される。チャンネル単位で
`memory: false` と書くと外れる (opt-out)。shared 無効時はこの値に関わらず配線されない
...
- **DM は default を継承しない**。予約名 `dm` エントリを別の土台として扱い、
  ...
  trigger (多くは mention) が DM にそのまま適用されないようにするため。
  `dm` エントリも無ければ DM は **disabled (起動しない)** — trigger.when が
  vacuously false ([]) の状態と同じ (§1 の表の通り)。1:1 の DM でメンションという
  操作は意味を持たないため、mention 相当の既定を作れず、明示的なオプトインを要求する。
- **DM を有効化したい場合**、`dm` エントリで `trigger.when` を明示指定する
  (例: `[{ kind: "passthrough" }]` で無条件起動、`[{ kind: "mention" }]` で
  メンション相当の運用)。
- **通常チャンネルも「未登録チャンネルからは一切起動しない」運用にしたい場合**は
  `default.trigger.when: []` にする (`evaluateWhen` は空配列を「vacuously false」
  として扱う。§7、gate.ts)。ただし default は全チャンネルの土台なので、個別に
  許可したいチャンネルは trigger.when を持つ固有エントリで明示的に上書きする必要がある。
...
pi (agent) 本体の `model` は pi の shorthand と同じ **canonical
`provider/model-id[:thinking-level]` 形式**で書く
(例: `google-vertex/gemini-3.5-flash`, `anthropic/claude-opus-4-8:high`,
`openrouter/moonshotai/kimi-k2.6`)。値は pi の `--model` にそのまま渡し、
provider の切り替え・thinking-level の解釈・モデル名の解決は pi に委譲する。
bridge 側の規則は 2 つだけ:

- **provider prefix (`/`) は必須** (schema の refine で fail-loud)。bare id は pi 側の
  fuzzy match で解決先 provider が非決定になり、下記 ADC marker の判定もできない。
- **認証は pi に委譲する**。API キー系 provider は pi が provider ごとに固定の env 名
  (`ANTHROPIC_API_KEY` / `OPENROUTER_API_KEY` 等) を読むので、`agent.env` (§5) で
  その名前を列挙して渡す。google-vertex だけは pi の env 判定がファイルベース ADC
  前提で Cloud Run の metadata-server ADC を認識しないため、bridge が model の
  provider prefix を見て `--api-key gcp-vertex-credentials` marker を付ける
  (runtime.ts の `ADC_MARKER_PROVIDERS`。ADC 系 provider を足すときはこの map に追記)。

classifier ノードの `model` はこの形式の対象外 (bridge 内蔵の判定用で、別機構)。
...
- **起動は検出済み entrypoint 経由**: pi の entrypoint は `agent.runtime.permissionMode` の
  有無に関係なく `resolvePiPaths` の自動検出値を使う。既定 ON のときは
  ...
  限る。`permissionMode: false` のときは permission flags を付けず、
  `node <entrypoint> <pi 引数>` で起動する。詳細は [session-runtime.md](session-runtime.md) §6
- **native addon の許可は別 opt-in**: `agent.runtime.allowAddons` (既定 false) を
  true にすると `--allow-addons` を追加する。native addon (.node) は fs チェックを
  素通りできるため、native 依存を持つ extension (例: pi-smart-fetch の wreq-js) を
  使う場合だけ明示的に緩める
...
agent:
  ...
  runtime:                # コンテナイメージの作り方と対になる実行環境設定
    ...
    allowAddons: ${env.PI_ALLOW_ADDONS:-false}  # native addon 拡張用の --allow-addons。既定 OFF
...
boot ブロックに**モデルも provider も置かない**。pi 本体のモデルは `channels` の
default.model / channel.model に canonical `provider/model-id[:thinking-level]` 形式で、
classifier のモデルは `trigger.when` の各 classifier ノードの `model` に書く (§2.3)。
service 単位のモデル既定という中間層を持たないので、boot ブロックには connector /
store / agent.turnTimeoutMs / agent.progressNoticeIntervalMs / agent.env /
agent.runtime のような**モデル以外の bridge 構成**だけが残る。

agent の一部フィールド (turnTimeoutMs / progressNoticeIntervalMs / runtime.*)
は env も直接 override できる。優先順位は **env > agent.yaml > コード既定**
(TURN_TIMEOUT_MS / PROGRESS_NOTICE_INTERVAL_MS / PI_AGENT_UID・GID /
PI_PERMISSION_MODE / PI_ALLOW_ADDONS / PI_AGENT_HOME が対応。resolveAgentConfig)。
...
channel: #alerts
model:            google-vertex/gemini-3-pro   ← channel
...
| bridge の挙動既定 (turn timeout / agent.runtime。モデル・provider は含まない) | 設定リポジトリ | agent.yaml の agent (§6) | YAML |
