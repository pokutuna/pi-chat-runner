...
- **EventSource (Trigger)** — HTTP push (Events API) / WebSocket (Socket Mode) という
  「イベントの届き方 + ACK の仕方」。環境で差し替える ([architecture.md](architecture.md) §1)
- **IngressAdapter (下記)** — 届いた生ペイロードを `ChatEvent` に正規化する
  プラットフォーム方言の変換器。EventSource の実装が内部で使う

つまり `HttpEventSource` と `SocketEventSource` は同じ `SlackIngressAdapter` を共有し、
違うのは受信経路と ACK だけ。プラットフォーム追加は IngressAdapter を、
受信経路追加は EventSource を書く。
...
```
Slack ──(Events API HTTP / Socket Mode WS)──▶ EventSource (Trigger)
...
| 常駐接続 | Socket Mode / discord.py 常駐 | EventSource で差し替え: 本番 Events API (HTTP push)、ローカル/お試しは Socket Mode | 本番は scale-to-zero と両立させ、開発は公開 URL なしで回す |
