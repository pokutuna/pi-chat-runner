...
入口を `EventSource` で抽象化し、後段を共通化する:
...
class HttpEventSource implements EventSource {}    // Events API。/slack/events で受ける (本番)
class SocketEventSource implements EventSource {}  // Socket Mode。WebSocket (ローカル/お試し)
```
...
- 実行モデル (レスポンス後に always-allocated で drain) は Cloud Run + Events API 前提。
  ...
  この差は EventSource 実装が吸収し、セッション層 ([session-model.md](session-model.md)) は触らない。
...
// 人が書くのはリポジトリ内 YAML で、CLI の apply がこの doc に変換して書き込む ([config.md](config.md) §6)
interface ChannelDoc {
  systemPrompt?: string;          // app 共通プロンプトへの追記分
  context?: string[];             // 短い参照テキスト。初回ターンに注入 ([config.md](config.md) §4)
  // 起動判定は Gate の合成 ([session-model.md](session-model.md) §5)。gates に種別+パラメータを並べ、combinator で束ねる。
  trigger?: {
    gates: Array<{ kind: "mention" | "keyword" | "classifier" | "passthrough" | "cooldown";
                   criteria?: string; pattern?: string }>;  // kind ごとに要るパラメータだけ
    combinator: "any" | "all";     // any=どれか / all=全て
    debounceSec?: number;
    cooldownSec?: number;
  };
  model?: string;                 // 省略時は gemini-3-pro
...
- 3 秒 ACK (経路で意味が違う。[EventSource](#1-全体構成--単一サービスcloud-tasks-なし) §1) /
...
1. **MVP**: 単一サービス。入口は `EventSource` で抽象化し、まず **Socket Mode でローカル確認**
