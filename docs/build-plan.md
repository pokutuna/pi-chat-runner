...
- [x] 実効設定の dump — スタンドアロン CLI ではなく bot の起動オプション
  (`node server.js dump <channel> [--json]`) として実装。ランタイムと同じ
  `resolveChannelConfig` + provenance を共有する ([design/config.md](design/config.md) §6)
- [x] classifier Gate (LLM 分類、`@google/genai` + Vertex、fail-closed。
  モデルは `trigger.when` の classifier ノード `model` / コード既定。
  [design/session-model.md](design/session-model.md) §5)
...
- [ ] resume_pending を伴う再開設計・鮮度窓 ([design/session-model.md](design/session-model.md) §6)。
  現状の「再開」は同じ `--session` パスでの再 spawn による transcript 継続のみ
  ([src/session/runner.ts](../src/session/runner.ts) の `resumed` フラグ) で、`SessionDoc.status` は
  `active`/`finished` の 2 値のみ (resume_pending/suspended 台帳も鮮度窓もなし)。
...
- [ ] リアクションによる再開 (`targetIsOwnMessage` からの逆引き。[design/chat-model.md](design/chat-model.md) §2.3)。
  ...
  逆引きに要る `chat_ref` 台帳 (上記 SessionEntry) 自体も未着手
...
│   ├── gate/
│   │   ├── gate.ts         # Gate IF + registry + trigger.when ブール木の評価
│   │   └── gates/          # mention.ts / keyword.ts / passthrough.ts / classifier.ts
│   ├── classifier/         # client.ts (GeminiClassifierClient, Vertex 経由の LLM 分類)
│   ├── config/             # agent-config.ts / channel-doc.ts / config-source.ts / dump.ts
│   ├── session/
